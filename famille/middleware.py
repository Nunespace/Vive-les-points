...
class SessionDebugMiddleware:
    ...
    def __call__(self, request):
        ...
        logger.warning(f"[SESSION DEBUG] Avant vue: id={session_key} auth={is_auth}")
        ...
        if hasattr(request, "session"):
            ...
            def flush_with_log(*args, **kwargs):
                logger.error(
                    ...
                    "".join(traceback.format_stack(limit=15))  # stack courte (15 lignes)
                )
        ...
        session_key = getattr(getattr(request, "session", None), "session_key", None)
        is_auth = getattr(getattr(request, "user", None), "is_authenticated", False)
        logger.warning(f"[SESSION DEBUG] Après vue: id={session_key} auth={is_auth}")
